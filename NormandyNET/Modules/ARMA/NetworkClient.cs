...
namespace NormandyNET.Modules.ARMA
{
    internal class NetworkClient
    {
        ...
        {
                        var address = Memory.Read<ulong>(networkClient + ModuleARMA.offsetsARMA.ServerName);

            if (address != 0)
            {
                serverName = ArmaString.GetString(address);
            }
        ...
        {
                        playersCount = Memory.Read<uint>(networkClient + ModuleARMA.offsetsARMA.ScoreBoard + 0x8);
                    }

        internal static void GetScoreBoard()
        {
                        scoreBoard = Memory.Read<ulong>(networkClient + ModuleARMA.offsetsARMA.ScoreBoard);
                    }
        ...
        internal static void GetScoreboardList()
        {
            ...
            for (uint i = 0; i < playersCount; i++)
            {
                ...
                var pIdentity = scoreBoard + i * ModuleARMA.offsetsARMA.PlayerIdentitySize;
                
                var player_network_id = Memory.Read<uint>(pIdentity + ModuleARMA.offsetsARMA.PlayerIdentitySizeNetworkId);
